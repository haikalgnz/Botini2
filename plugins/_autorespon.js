...
`.trim(), watermark, 'Pemilik Bot', ',owner', 'Cara memasukkan bot', '.how add', 'Sewa', '.sewa',m)
    ...
    if (setting.statusupdate) {
        if (new Date() * 1 - setting.status > 1000) {
            ...
            await this.setStatus(`Aktif selama ${uptime} | Mode: ${global.opts['self'] ? 'Private' : setting.groupOnly ? 'Hanya Grup' : 'Publik'} | Ikal bot oleh Haikal | Mau join kan bot Ketik  .join linkgc`).catch(_ => _) 
